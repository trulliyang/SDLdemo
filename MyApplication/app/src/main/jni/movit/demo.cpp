...
{
	SDL_Surface *img = SDL_LoadBMP(filename);// IMG_Load(filename);
	if (img == NULL) {
...
string readafile(string filename){
	string aa(source_path);
    const string full_pathname = aa + "/" + filename;
...
//	string footer = read_file("footer.frag");
//    readafile("footer.frag");
...
//    while(true){
//
//        glClearColor(1.0, grey, grey, 1.0f);
...
//    sprintf(file_path,"%s",source_path);
	std::string sourcea(source_path) ;
//    sourcea.insert(0,source_path);
	CHECK(init_movit(sourcea, MOVIT_DEBUG_ON));

//	string fragment_shader_processed = readafile("chain-003.frag");
...
//	unsigned char *src_img = load_image(argc > 1 ? argv[1] : "blg_wheels_woman_1.jpg", &img_w, &img_h);
	unsigned char *src_img = load_image(file_path, &img_w, &img_h);
