#include "watermark_overlay_effect.h"
#include "util.h"
...
namespace movit {
	...
	WatermarkOverlayEffect::WatermarkOverlayEffect()
	: swap_inputs(false)
{
	__android_log_print(ANDROID_LOG_ERROR, "shiyang", "shiyang WatermarkOverlayEffect Constructor");
	...
	alpha = 0.5;
	register_float("alpha", &alpha);
}

string WatermarkOverlayEffect::output_fragment_shader()
{
	__android_log_print(ANDROID_LOG_ERROR, "shiyang", "shiyang WatermarkOverlayEffect output_fragment_shader");
	char buf[256];
	snprintf(buf, sizeof(buf), "#define SWAP_INPUTS %d\n", swap_inputs);
    return buf + read_file("watermark_overlay_effect.frag");
}
	
void WatermarkOverlayEffect::set_gl_state(GLuint glsl_program_num,
										  const string &prefix,
