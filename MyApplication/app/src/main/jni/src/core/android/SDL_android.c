...
JNIEXPORT void JNICALL SDL_JAVA_INTERFACE(nativeSetupJNI)(
        JNIEnv* mEnv, jclass cls, jstring path);
...
JNIEXPORT void JNICALL SDL_JAVA_INTERFACE(nativeSetupJNI)(JNIEnv* mEnv, jclass cls ,jstring path)
{
    __android_log_print(ANDROID_LOG_VERBOSE, "SDL", "nativeSetupJNI()");

    const char *c_str = NULL;
    c_str = (*mEnv)->GetStringUTFChars(mEnv, path,0);
    strcpy(source_path,c_str);
    (*mEnv)->ReleaseStringUTFChars(mEnv, path, c_str);
