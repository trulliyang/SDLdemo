...
//		return NULL;
	...
	extern void avformat_mlt_register( mlt_repository repository,char* yml_path);

	avformat_mlt_register( self ,directory);
	// Register the object file for closure
	mlt_properties_set_data( &self->parent, "avformat", NULL, 0, ( mlt_destructor )dlclose, NULL );

	extern void sdl_mlt_register( mlt_repository repository,char* yml_path);

	sdl_mlt_register( self ,directory);

	extern  void opengles_mlt_register( mlt_repository repository);
	opengles_mlt_register( self );

	// Register the object file for closure
	mlt_properties_set_data( &self->parent, "sdl", NULL, 0, ( mlt_destructor )dlclose, NULL );
