...
void mlt_pool_stat( )
{
	...
	__android_log_print(ANDROID_LOG_INFO, "mlt_pool_stat", " mlt_pool_stat %s: count %d\n", __FUNCTION__, c);
	...
	for ( i = 0; i < c; i ++ )
	{
		...
		__android_log_print(ANDROID_LOG_INFO, "mlt_pool_stat", " mlt_pool_stat %s: size %d allocated %d returned %d %c\n", __FUNCTION__,
						 pool->size, pool->count, mlt_deque_count( pool->stack ),
						 pool->count !=  mlt_deque_count( pool->stack ) ? '*' : ' ' );
	...
	__android_log_print(ANDROID_LOG_INFO, "mlt_pool_stat", " mlt_pool_stat %s: allocated %"PRIu64" bytes, used %"PRIu64" bytes \n",
					 __FUNCTION__, allocated, used );
}
