...
int mlt_consumer_init( mlt_consumer self, void *child, mlt_profile profile )
{
	...
	if ( error == 0 )
	{
		...
		priv->image_format = mlt_image_yuv420p;//mlt_image_yuv422;
...
static void *consumer_read_ahead_thread( void *arg )
{
	...
	int buffer = 10;//mlt_properties_get_int( properties, "buffer" ) + 1;
...
static void consumer_work_start( mlt_consumer self )
{
	...
	// Create the read ahead
	if (1) // mlt_properties_get( MLT_CONSUMER_PROPERTIES( self ), "priority" ) )
	{

		struct sched_param priority;
		pthread_attr_t thread_attributes;

		priority.sched_priority = 15 ;//mlt_properties_get_int( MLT_CONSUMER_PROPERTIES( self ), "priority" );
...
//			priv->consecutive_dropped, priv->consecutive_rendered, priv->process_head );
		...
		{
			int orig_buffer = 10;//mlt_properties_get_int( properties, "buffer" );
			int prefill = 10;//mlt_properties_get_int( properties, "prefill" );
...
//    __android_log_print(ANDROID_LOG_ERROR, "ProjectName", "need a frame render mlt_consumer_rt_frame  %d !!!",mlt_deque_count(priv->queue));
	...
	else if ( priv->real_time == 1 || priv->real_time == -1 )
	{
		...
		if ( priv->preroll )
		{
			int buffer = 10;//mlt_properties_get_int( properties, "buffer" );
			int prefill = 10;//mlt_properties_get_int( properties, "prefill" );
#ifndef _WIN32
			consumer_read_ahead_start( self );
#endif
