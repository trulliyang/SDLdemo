...
int main( int argc, char **argv )
{
	...
	char media_file[1024] = {0};
    sprintf(media_file,"%s%s%s","avformat:","mnt/sdcard/","bbc-japan_1080p.mp4");
    argc = 2;
    argv[0] = "melt";
    argv[1] = media_file;
...
//	FILE* fp = fopen("mnt/sdcard/bbc-japan_1080p.mp4","r");
	...
	fprintf(stderr, "argc %d argv %s",argc, *argv);
...
//	avformat_init();
	...
	char dd[1024]={0};
	sprintf(dd,"%s/%s",source_path,"producer_avformat.yml");

	extern mlt_producer producer_avformat_init( mlt_profile profile, const char *service, char *file );
   extern  void avformat_init( );
    avformat_init();

	melt = producer_avformat_init(  profile, "melt",  dd);

	extern mlt_consumer consumer_sdl_init( mlt_profile profile, mlt_service_type type, const char *id, char *arg );
	sprintf(dd,"%s/%s",source_path,"consumer_sdl.yml");
	consumer =  consumer_sdl_init( profile, (mlt_service_type)8 ,"melt", dd );
