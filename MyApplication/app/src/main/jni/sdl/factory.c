...
//MLT_REPOSITORY
void sdl_mlt_register( mlt_repository repository,char* yml_path)
{
	MLT_REGISTER( consumer_type, "sdl", consumer_sdl_init );
    char yml[PATH_MAX] = {0};
    sprintf(yml,"%s/%s",yml_path,"consumer_sdl.yml");
	MLT_REGISTER_METADATA( consumer_type, "sdl", metadata, yml );
