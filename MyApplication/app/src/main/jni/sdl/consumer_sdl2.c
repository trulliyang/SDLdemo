...
static int setup_sdl_video( consumer_sdl self )
{
	...
	int texture_format = SDL_PIXELFORMAT_IYUV;//SDL_PIXELFORMAT_YUY2;
...
//	mlt_image_format vfmt = mlt_properties_get_int( properties, "mlt_image_format" );
	mlt_image_format vfmt = mlt_image_yuv420p;////mlt_image_yuv422;
...
//		__android_log_print(ANDROID_LOG_ERROR, "ProjectName", "need a frame render before11111   !!!");
		...
		if ( self->running && image )
		{
			...
			if ( strides[1] ) {
//				__android_log_print(ANDROID_LOG_ERROR, "ProjectName", "need a frame render before3333  !!!");

				SDL_UpdateYUVTexture( self->sdl_texture, NULL,
					planes[0], strides[0],
					planes[1], strides[1],
					planes[2], strides[2] );
//                __android_log_print(ANDROID_LOG_ERROR, "ProjectName", "need a frame render before22222   1111111  !!!");
...
//		__android_log_print(ANDROID_LOG_ERROR, "ProjectName", "need a frame render before22222   !!!");
...
static void *video_thread( void *arg )
{
	...
	while ( self->running )
	{
		...
		if ( mlt_properties_get_int( properties, "rendered" ) == 1 && self->running )
		{
            ...
            __android_log_print(ANDROID_LOG_ERROR, "ProjectName", "need a frame render before!!!");
            ...
            __android_log_print(ANDROID_LOG_ERROR, "ProjectName", "need a frame render finished!!!");
		...
		{
            __android_log_print(ANDROID_LOG_ERROR, "ProjectName", "need a frame dropped video frame!!!");
