...
//				if ( producer_open( self, profile, "mnt/sdcard/bbc-japan_1080p.mp4", 1, 1 ) != 0 )
...
//	filename = "mnt/sdcard/4.png";//        m.set_uri(\"mnt/sdcard/5.mp3\");";
//
//    __android_log_print(ANDROID_LOG_INFO, "ProjectName", "avformat_open_input begin   %d  duration  %d ======== !!!" ,error);


//	AVFormatContext* par = avformat_alloc_context();
//	if(avformat_open_input(&par, filename, NULL,NULL)!=0){
//		int ii = 32;
//	}
//
//	if(avformat_find_stream_info(par, NULL)<0){
//
//
//		avformat_close_input(&par);
//		avformat_free_context(par);
//	}
//
//	for(int i=0; i<par->nb_streams; i++){
//		if(par->streams[i]->codecpar->codec_type==AVMEDIA_TYPE_VIDEO){
//
//			avformat_close_input(&par);
//			avformat_free_context(par);
//		}
//	}
//
//    error = avformat_open_input( &self->video_format, filename, NULL, NULL ) ;
//    error = avformat_find_stream_info(self->video_format,NULL);
//    for(int i =0;i<self->video_format->nb_streams;i++){
//        if(AVMEDIA_TYPE_VIDEO == self->video_format->streams[i]->codec->codec_type){
//            __android_log_print(ANDROID_LOG_INFO, "ProjectName", "avformat_open_input    %d  duration  %d   %lld ======== !!!" ,
//                                self->video_format->streams[i]->codec->width,self->video_format->streams[i]->codec->height,			self->video_format->duration);
//
//
////			int got_picture = 0;
////		avcodec_decode_video2( self->video_format, self->video_frame, &got_picture, &self->pkt );
//
//		}
//    }
//    avformat_close_input( &self->video_format );
//    __android_log_print(ANDROID_LOG_INFO, "ProjectName", "avformat_open_input end   %d  duration  %d ======== !!!" ,error);
...
//
////		int  ll = ( end.tv_sec * 1000000 + end.tv_usec ) - ( start.tv_sec * 1000000 + start.tv_usec );
//
//		__android_log_print(ANDROID_LOG_INFO, "ProjectName", "avformat_open_input    %d  duration  %d ======== !!!" ,error);
//
//	}
		...
		while( ret >= 0 && !got_picture )
		{
			...
			if ( self->pkt.stream_index == self->video_index )
			{
				...
				if ( must_decode  || int_position >= req_position || !self->pkt.data )
				{
                   ...
                   // __android_log_print(ANDROID_LOG_INFO, "ProjectName", "need a frame render before44444 ======== !!!");
					...
					int  ll = ( end.tv_sec * 1000000 + end.tv_usec ) - ( start.tv_sec * 1000000 + start.tv_usec );
					if(ll >500)
                    __android_log_print(ANDROID_LOG_INFO, "ProjectName", "need a frame render before44444  %d  duration  %d ======== !!!" ,got_picture,ll);
...
//				__android_log_print(ANDROID_LOG_ERROR, "ProjectName", "need a frame render before222222  %d !!!",got_picture);
...
//					__android_log_print(ANDROID_LOG_ERROR, "ProjectName", "need a frame render before33333222222-------  %d !!!",got_picture);

//					if ( int_position < req_position )
//						got_picture = 0;
//					else if ( int_position >= req_position )
						codec_context->skip_loop_filter = AVDISCARD_NONE;
//					__android_log_print(ANDROID_LOG_ERROR, "ProjectName", "need a frame render before3333311111111-------  %d  int_position  %lld  req_position %lld!!!",got_picture,int_position,req_position);
...
//			__android_log_print(ANDROID_LOG_ERROR, "ProjectName", "need a frame render before33333333-------  %d !!!",got_picture);
...
//					__android_log_print(ANDROID_LOG_ERROR, "ProjectName", "need a frame render before666666   !!!");

//					yuv_colorspace = convert_image( self, self->video_frame, *buffer, codec_context->pix_fmt,
//						format, *width, *height, &alpha );

					yuv_colorspace = self->yuv_colorspace;

					uint8_t * dst = *buffer;
					for(int i = 0; i < *height; i++)
						{
						  memcpy(dst+*width*i, self->video_frame->data[0]+self->video_frame->linesize[0]*i,*width);
						}

					dst = (dst + *width**height);
					for(int j = 0; j <*height; j++)
						{
							memcpy(dst+*width/2*j, self->video_frame->data[1]+self->video_frame->linesize[1]*j, *width/2);
						}
					dst = (dst + *width**height/4);
					for(int k =0; k < *height; k++)
						{
							memcpy(dst+ *width/2*k, self->video_frame->data[2]+self->video_frame->linesize[2]*k, *width/2);
						}
//					memcpy(*buffer,self->video_frame->data[0],(*width**height*3)>>1);
//                    avpicture_fill((AVPicture*)frame, *buffer, AV_PIX_FMT_YUV420P, *width, *width);
//                    yuv_colorspace = self->yuv_colorspace;
//                    __android_log_print(ANDROID_LOG_ERROR, "ProjectName", "need a frame render before77777   !!!");
